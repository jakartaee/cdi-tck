...
public class PersistenceInjectionStaticProducerFieldTest extends AbstractTest {

    @Deployment
    public static WebArchive createTestArchive() {
        return new WebArchiveBuilder().withTestClassPackage(PersistenceInjectionStaticProducerFieldTest.class)
                .withDefaultPersistenceXml().build();
    }

    @Test(groups = { INTEGRATION, PERSISTENCE })
    @SpecAssertion(section = DECLARING_RESOURCE, id = "cc")
    ...
    @Test(groups = { INTEGRATION, PERSISTENCE })
    @SpecAssertion(section = DECLARING_RESOURCE, id = "dd")
