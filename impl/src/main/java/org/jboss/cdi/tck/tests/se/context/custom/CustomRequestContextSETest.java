...
public class CustomRequestContextSETest extends Arquillian {
    ...
    @SpecAssertions({ @SpecAssertion(section = Sections.BUILTIN_CONTEXTS, id = "a"), @SpecAssertion(section = Sections.BUILTIN_CONTEXTS, id = "b"),
            @SpecAssertion(section = Sections.BUILTIN_CONTEXTS, id = "ea"), @SpecAssertion(section = Sections.BUILTIN_CONTEXTS, id = "eb"),
            @SpecAssertion(section = Sections.BUILTIN_CONTEXTS, id = "ec") })
    public void defineCustomRequestContext() {
        try (SeContainer seContainer = SeContainerInitializer.newInstance().initialize()) {
            ...
            assertTrue(CustomRequestContextObserver.initializedCalled);
        }

        assertTrue(CustomRequestContextObserver.beforeDestroyedCalled);
        assertTrue(CustomRequestContextObserver.destroyedCalled);
    }
}
