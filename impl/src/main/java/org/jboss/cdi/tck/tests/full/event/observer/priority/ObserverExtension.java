...
public class ObserverExtension implements Extension {
    ...
    public void registerObservers(@Observes ProcessObserverMethod<?, ?> processObserverMethod) {
        ...
        String id = observerMethod.getBeanClass().getSimpleName() + "." + processObserverMethod.getAnnotatedMethod().getJavaMember().getName();

        observerMethodPriorities.put(id, observerMethod.getPriority());
    }

    public Integer getObserverMethodPriority(String id) {
        return observerMethodPriorities.get(id);
    }
}
