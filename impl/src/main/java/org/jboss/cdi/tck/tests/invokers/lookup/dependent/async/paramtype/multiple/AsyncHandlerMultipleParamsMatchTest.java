...
public class AsyncHandlerMultipleParamsMatchTest extends AbstractTest {
    @Deployment
    public static WebArchive createTestArchive() {
        return new WebArchiveBuilder()
                .withTestClassPackage(AsyncHandlerMultipleParamsMatchTest.class)
    ...
    @SpecAssertion(section = Sections.INVOKER_ASYNCHRONOUS_METHODS, id = "ce")
    @SpecAssertion(section = Sections.INVOKER_ASYNCHRONOUS_METHODS, id = "f")
    public void test(InvokerHolder invokers) throws Exception {
        MyDependentBean.reset();

        Invoker<MyBean, MyAsyncType> hello = invokers.get("hello");
        CompletableFuture<String> future = new CompletableFuture<>();

        assertEquals(MyDependentBean.destroyedCounter.get(), 0);

        hello.invoke(null, new Object[] { null, future, null, null });

        // _not_ async (multiple matching parameters), so destroyed synchronously
        assertFalse(MyBean.futureComplete);
        assertEquals(MyDependentBean.destroyedCounter.get(), 1);

        future.complete("hello");

        assertTrue(MyBean.futureComplete);
        assertEquals(MyDependentBean.destroyedCounter.get(), 1);
    }
}
