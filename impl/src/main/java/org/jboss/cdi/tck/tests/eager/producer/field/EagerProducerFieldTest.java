...
public class EagerProducerFieldTest extends AbstractTest {
    // note that for this test, each producer field must live in its own bean, to avoid
    // initialization of _all_ producer fields when value of _one_ is requested

    @Deployment
    public static WebArchive createTestArchive() {
        return new WebArchiveBuilder()
                .withTestClassPackage(EagerProducerFieldTest.class)
                .build();
    }

    @Test
    @SpecAssertion(section = EAGER_INITIALIZATION, id = "ac")
