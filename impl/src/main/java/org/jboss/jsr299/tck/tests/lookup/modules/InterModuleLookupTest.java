...
public class InterModuleLookupTest extends AbstractJSR299Test {

    @Deployment
    public static EnterpriseArchive createTestArchive() {

        EnterpriseArchive enterpriseArchive = new EnterpriseArchiveBuilder().noDefaultWebModule()
                .withTestClassDefinition(InterModuleLookupTest.class).withClasses(ManagedFoo.class).withBeanLibrary(Foo.class)
                .build();
        enterpriseArchive.addAsModule(new WebArchiveBuilder().notTestArchive().withDefaultEjbModuleDependency()
                .withClasses(InterModuleLookupTest.class, WebFooLookup.class).build());
