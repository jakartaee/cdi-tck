...
|Technology Under Test  +|
                     Specifications and the compatible implementation for Jakarta Contexts and Dependency Injection Version {revnumber}.
|Test Suite   +|
...
*  *Testing platform* - The CDI TCK requires version {arquillian-version} of the Arquillian (https://arquillian.org). The TCK test suite is based on TestNG {testng-version} (https://testng.org).


*  *Porting Package* - An implementation of SPIs that are required for the test suite to run the in-container tests and at times extend the CDI {revnumber} API to provide extra information to the TCK.
...
[TIP]
====
The TCK distribution includes +weld/porting-package-lib/weld-inject-tck-runner-X.Y.Z-Q-tests.jar+ which contains two classes showing how the Weld compatible implementation passes the CDI TCK. The source for these classes is available from link:$$https://github.com/weld/core/tree/{weld-version}/inject-tck-runner/src/test/java/org/jboss/weld/atinject/tck$$[]
...
CDI supports {jakartaee-version}, {jakartaee-version} Web Profile, and {jakartaee-version} Core Profile. The TCK will execute on any of these runtimes, but is only part of the CTS for {jakartaee-version} and {jakartaee-version} Web Profile.

[[libraries]]

=== Packages for Jakarta Contexts and Dependency Injection Version {revnumber}
