...
----
      <plugin>
            ...
            <configuration>
              ...
              <dependenciesToScan>
                <dependency>jakarta.cdi:jakarta.cdi-tck-core-impl</dependency>
              </dependenciesToScan>
            </configuration>
       </plugin>
----
...
The CDI TCK is built atop Arquillian and TestNG, and it's TestNG that is responsible for selecting the tests to execute, the order of execution, and reporting the results. Detailed TestNG documentation can be found at link:$$https://testng.org/doc/documentation-main.html$$[testng.org].
...
Please note that this exclude list serves only for the previous micro version of TCK release! This means that if the latest version of TCK is e.g. 5.0.1 then this exclude list is valid only for the version 5.0.0 and invalid for any other version!
====
